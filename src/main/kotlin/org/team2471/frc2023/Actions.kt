...
suspend fun intakeCurrentLogic() {
    ...
    suspend fun intakeFromGround() = use(Arm, Intake) {
        if (OI.controlledBy == OI.personInControl.DRIVER) {
            println("driver in control")
        } else {
            OI.controlledBy = OI.personInControl.OPERATOR
...
//    }
    ...
    suspend fun toDrivePose() = use(Arm, Intake) {
        if (OI.controlledBy == OI.personInControl.DRIVER) {
            ...
            if (Arm.wristPosition.x < 0.0) animateToPose(Pose.BACK_DRIVE_POSE) else if (Arm.wristPosition.y > 0.0) animateToPose(
