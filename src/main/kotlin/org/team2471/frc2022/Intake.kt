...
object Intake : Subsystem("Intake") {
    ...
    var pivotOffset = -19.15  //comp: if angle too high, increase offset 159.7   .0145
    ...
    var pivotAngle : Double = 0.0
        get() =  ((-1.0*(pivotEncoder.absolutePosition - 0.427) * 360.0 * 118.4 / 105.8) + pivotOffset).degrees.wrap().asDegrees  //0.573  // (if (isCompBot) -1.0 else 1.0) * (((pivotEncoder.absolutePosition - 0.334) * 360.0 * 90.0 / 86.0) + pivotOffset).degrees.wrap().asDegrees
...
//        }
    ...
    val PIVOT_INTAKE
        get() = 8.0 //if (isCompBot) intakePresetEntry.getDouble(defaultPivotIntake) else 16.0
    val PIVOT_STORE = if (isCompBot) 90.0 else 90.0
    val PIVOT_TOP = if (isCompBot) 90.0 else 90.0
